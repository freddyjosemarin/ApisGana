...
if (count($_POST)) {
  ...
  $sqlQuery = "CALL SELECT_REPORTE_LOTERIA_CHANCES (?, ?)";
  $stmtQuery = $GLOBALS['DB']->prepare($sqlQuery);
  $stmtQuery->execute(array($_POST['desde'], $_POST['hasta']));
  ...
  <div class="content-wrapper">
    <div class="content-header row">
      <div class="content-header-left col-md-12 col-12 mb-1">
        <h3 class="content-header-title" align="center">Reporte de Venta de Loteria y Chance</h3>
      </div>
    </div>
...
function showData() {
  ...
  $.post('repoVentaLoteria.php',params,function(retval){
    tabla.rows().remove();
    datos=retval.datos;
    ...
    for (n in datos) {
        ...
        tr.insertCell().innerText=item.fecha;
        tr.insertCell().innerText=item.punto;
        tr.insertCell().innerText=item.padre;
        tr.insertCell().innerText=item.sorteo;
        tr.insertCell().innerText=item.IdSorteo;
        tr.insertCell().innerText=item.Serie;
        tr.insertCell().innerText=item.Numero;
        tr.insertCell().innerText=item.Cantidad;
        tr.insertCell().innerText=parseFloat(item.Monto).toFixed(2);
        tr.insertCell().innerText=item.Transaccion;
        tr.insertCell().innerText=item.Orden;
        tr.insertCell().innerText=item.Celular;
        tr.insertCell().innerText=item.Email;
